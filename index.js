...
function openBindMultiDialog(ctx, settings) {
  ...
  const rows = [];
  for (let i = 0; i < (ctx.characters?.length || 0); i++) {
  ...
  box.querySelector("#abgm_multi_cancel")?.addEventListener("click", () => ov.remove());

  box.querySelector("#abgm_multi_apply")?.addEventListener("click", async () => {

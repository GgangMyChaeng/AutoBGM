...
console.log("[AutoBGM] module eval");
let extension_settings;
let saveSettingsDebounced;

async function __abgmResolveDeps() {
  // ✅ Some ST builds expose these on window; prefer that to avoid path issues.
  if (window?.extension_settings) extension_settings = window.extension_settings;
  if (window?.saveSettingsDebounced) saveSettingsDebounced = window.saveSettingsDebounced;

  if (extension_settings && saveSettingsDebounced) return;
  ...
  const extMod =
    ...
    (await tryImport("../../extensions.js")) ||
    (await tryImport("../../../extensions/extensions.js")) ||
    (await tryImport("../../extensions/extensions.js")) ||
    (await tryImport("../../../../extensions.js"));

  if (extMod?.extension_settings) extension_settings = extMod.extension_settings;

  const scriptMod =
    (await tryImport("../../../../script.js")) ||
    (await tryImport("../../../script.js")) ||
    (await tryImport("../../../../scripts/script.js")) ||
    (await tryImport("../../../scripts/script.js")) ||
    (await tryImport("../../../../public/script.js"));

  if (scriptMod?.saveSettingsDebounced) saveSettingsDebounced = scriptMod.saveSettingsDebounced;

  if (!extension_settings) {
    throw new Error("[AutoBGM] Failed to resolve extension_settings (extensions.js path mismatch)");
  }
  if (!saveSettingsDebounced) {
    throw new Error("[AutoBGM] Failed to resolve saveSettingsDebounced (script.js path mismatch)");
  }
}
...
function init() {
  // ✅ allow re-mount on hot reload / refresh-assets
  ...
  if (!window.__AUTOBGM_ENGINE_STARTED__) {
    window.__AUTOBGM_ENGINE_STARTED__ = true;
    try { startEngine(); } catch (e) { console.error("[AutoBGM] startEngine failed", e); }
  }

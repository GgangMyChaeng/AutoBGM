...
function getBoundCharacterIdsForPreset(ctx, presetId) {
  const out = [];
  try {
    ...
    for (let i = 0; i < chars.length; i++) {
      const ch = chars[i];
      const bound = ch?.data?.extensions?.[AUTOBGM_BIND_KEY]?.presetId;
      if (String(bound || "") === pid) out.push(i);
    }
  } catch {}
  return out;
}

function getCharacterNameById(ctx, cid) {
  try {
    const ch = ctx?.characters?.[cid];
    return String(ch?.name || ch?.data?.name || ch?.data?.avatar || `#${cid}`);
  } catch {
    return `#${cid}`;
  }
}
...
function openBindMultiDialog(ctx, settings) {
  ...
  const presetName = settings.presets?.[presetId]?.name || presetId;

  // 이미 이 프리셋에 종속된 캐릭들
  const boundIds = new Set(getBoundCharacterIdsForPreset(ctx, presetId));
  ...
  box.style.maxHeight = "min(620px, 78vh)";
  box.style.background = "rgba(10,10,10,.98)";
  box.style.color = "var(--SmartThemeTextColor, #eee)";
  ...
  box.style.gap = "10px";
  box.style.boxShadow = "0 12px 40px rgba(0,0,0,.55)";
  ...
  title.innerHTML = `
    ...
    <div style="opacity:.75; font-size:12px;">preset: ${escapeHtml(presetName)}</div>
  `;
  ...
  listWrap.style.background = "rgba(0,0,0,.25)";

  const rows = [];
  for (let i = 0; i < ctx.characters.length; i++) {
    const name = getCharacterNameById(ctx, i);
  ...
  actions.appendChild(btnApply); // ✅ 좌측 적용
  actions.appendChild(btnCancel); // ✅ 우측 취소
  ...
  btnApply.addEventListener("click", async () => {
    ...
    if (typeof writeExtensionField !== "function") {
      alert("writeExtensionField를 사용할 수 없음");
      return;
    }

    // ✅ 선택된 캐릭은 presetId로 세팅
    // ✅ 선택 해제된 캐릭 중에서 '이 프리셋'에 묶여있던 애는 해제
    for (const r of rows) {
      const want = !!r.cb.checked;
      const was = boundIds.has(r.cid);

      if (want && !was) {
        await writeExtensionField(r.cid, AUTOBGM_BIND_KEY, { presetId });
      } else if (!want && was) {
        await writeExtensionField(r.cid, AUTOBGM_BIND_KEY, { presetId: "" });
      }
    }
...
function initModal(overlay) {
  ...
  btnBindMulti?.addEventListener("click", () => {
    ...
    openBindMultiDialog(ctx, settings);
  });

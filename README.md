...
- 디버그 모드(Debug Mode): 활성화하면 지금 확장이 Ai의 마지막 메시지를 제대로 인식하고 있는지 "Now Playing"에서 확인할 수 있음. 기본적으로 보이는 건 "asstLen"고, 키워드 모드 활성화 시 "kw"와 "hit"가 추가로 보임.
...
- Import: (이름)_AutoBGM.json 파일을 삽입하면, 프리셋의 명칭·mp3 파일명·키워드·우선도·볼륨 설정만 불러와짐. 참고로 이미 있는 프리셋을 또 불러왔다면 중복되는 프리셋 뒤에 (imported)이 뜰 거임. 그리고 BGM을 위한 mp3 파일은 따로 개인이 삽입해 줘야 함. 해당 프리셋의 mp3 파일명과 일치하는 것들을 불러와주면 알아서 잘 기능할 거임.
...
- 프리셋 기능인
  ...
  * 특정 프리셋을 특정 캐릭터(들)에게 매칭(종속; 쉽게 말해 사용자가 해당 캐릭터 카드를 눌렀을 시 자동 연결해주는 기능)하는 버튼이 있는가? (O)
### @BGM list에서...@
- BGM list의 기능인
...
- AutoBGM의 기본 기능인
  ...
  * 특정 프리셋을 특정 캐릭터(들)에게 매칭(종속)하는 기능이 작동하는가? (O)
- 키워드 모드 활성화 중에...
